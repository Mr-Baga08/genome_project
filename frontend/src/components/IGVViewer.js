// frontend/src/components/IGVViewer.js
    ...
    try {
      ...
      if (selectedFiles.alignment) {
        options.tracks.push({
          ...
          indexURL: selectedFiles.alignment.download_url.replace('.bam', '.bam.bai'), // Assume index exists
      ...
      const browser = await igv.createBrowser(igvContainerRef.current, options);
      
      // Set initial locus if available
      if (options.tracks.length > 0) {
        await browser.search('chr1:1-100000'); // Default view
      }
      ...
      actions.addNotification({
        type: 'error',
        title: 'IGV Error',
        message: `Failed to initialize genome browser: ${error.message}`
      });
    } finally {
      setIsLoading(false);
    }
